...
def test_censor_calls_attempt_with_verify_false_and_single_attempt(tmp_path):
    ...
    with patch.object(proc, "_find_srt_file", return_value="/does/not/matter.srt"), \
         patch.object(proc, "_parse_srt_file", return_value=[make_subtitle_with_profanity()]), \
         patch.object(proc, "_attempt_censoring_with_fallback", return_value=(True, "out.mp4", [(0.0, 1.0, -100.0)], 1, [])) as mock_attempt:
...
def test_censor_calls_attempt_with_verify_true_and_three_attempts(tmp_path):
    ...
    with patch.object(proc, "_find_srt_file", return_value="/does/not/matter.srt"), \
         patch.object(proc, "_parse_srt_file", return_value=[make_subtitle_with_profanity()]), \
         patch.object(proc, "_attempt_censoring_with_fallback", return_value=(True, "out.mp4", [(0.0, 1.0, -60.0)], 3, [])) as mock_attempt:
...
def test_full_mode_executes_astats_and_parses_rms(tmp_path):
    ...
    with patch.object(proc, "_find_srt_file", return_value="/does/not/matter.srt"), \
         patch.object(proc, "_parse_srt_file", return_value=subs), \
         patch.object(proc, "_construct_ffmpeg_command", return_value=["ffmpeg", "-i", "in.mp4", "out.mp4"]) as mock_construct:
