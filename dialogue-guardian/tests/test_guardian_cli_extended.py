...
class TestGuardianCLIExtended(unittest.TestCase):
    ...
    def test_parser_help_message(self):
        ...
        self.assertIn("video_file", help_text)
    ...
    def test_setup_logging_with_file_handler_error(self, mock_stderr, mock_file_handler, mock_basic_config):
    ...
    def test_main_with_logging_options(self):
        ...
        with patch("guardian.cli.validate_args") as mock_validate, \
             patch("guardian.cli.setup_logging") as mock_setup_logging, \
             patch("guardian.cli.GuardianProcessor") as mock_processor_class, \
             patch("os.path.exists", return_value=True), \
             patch("os.path.abspath", return_value="/abs/test.mp4"), \
    ...
    def test_main_with_custom_ffmpeg_path(self):
        ...
        with patch("guardian.cli.validate_args") as mock_validate, \
             patch("guardian.cli.setup_logging"), \
             patch("guardian.cli.GuardianProcessor") as mock_processor_class, \
             patch("os.path.exists", return_value=True), \
             patch("os.path.abspath", return_value="/abs/test.mp4"), \
             patch("builtins.print"):
    ...
    def test_main_with_exception_during_processing(self):
        ...
        with patch("guardian.cli.validate_args") as mock_validate, \
            ...
            mock_validate.return_value = True
            mock_processor = MagicMock()
            mock_processor.process_video.side_effect = ValueError("Invalid video format")
            mock_processor_class.return_value = mock_processor
    ...
    def test_create_parser_argument_groups(self):
        ...
        optional_actions = [action for action in parser._actions if not action.required and action.dest != 'help']
