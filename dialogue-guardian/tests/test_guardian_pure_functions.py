...
class TestGuardianPureFunctions(unittest.TestCase):
    ...
    def test_parse_audio_streams_multiple_streams(self):
        """Test parsing multiple audio streams - should pick the one with most channels"""
    ...
    def test_build_volume_filters_custom_quote(self):
        ...
        result = self.processor._build_volume_filters(segments, volume_setting, quote_char)
    ...
    def test_build_audio_filter_chain_basic_strategy(self):
        ...
        expected = "volume=0:enable='between(t,1.0,2.0)',volume=0:enable='between(t,5.0,6.0)'"
        self.assertEqual(result, expected)
    ...
    def test_build_audio_filter_chain_enhanced_strategy(self):
        ...
        self.assertIn("acompressor=threshold=-20dB:ratio=20:attack=5:release=50", result)
    ...
    def test_build_audio_filter_chain_no_segments(self):
        ...
        # Should still include format normalization and compression, but no volume filters
        self.assertIn("aformat=sample_fmts=s16:channel_layouts=stereo", result)
        self.assertIn("acompressor=threshold=-20dB:ratio=20:attack=5:release=50", result)
    ...
    def test_build_ffmpeg_base_command(self):
        ...
        result = self.processor._build_ffmpeg_base_command(video_path, output_path, audio_filter)
