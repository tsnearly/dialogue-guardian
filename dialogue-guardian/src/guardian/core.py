...
class GuardianProcessor:
    ...
    def __init__(self, matching_words: Optional[List[str]] = None, 
                 ffmpeg_cmd: str = 'ffmpeg', ffprobe_cmd: str = 'ffprobe'):
        """
    ...
    def get_video_details(self, filename: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Get video duration
            cmd_duration = [self.ffprobe_cmd, "-v", "error", "-show_entries", 
            ...
            cmd_audio = [self.ffprobe_cmd, "-v", "error", "-select_streams", "a", 
                        "-show_entries", "stream=codec_name,channels,channel_layout,sample_rate", 
                        "-of", "compact=p=0:nk=1", filename]
            ...
            cmd_video = [self.ffprobe_cmd, "-v", "error", "-select_streams", "v:0", 
    ...
    def censor_audio_with_ffmpeg(self, video_path: str, output_path: Optional[str] = None) -> Optional[str]:
        """
        ...
        if not subs:
            ...
            pattern = r'\b(' + '|'.join(re.escape(word) for word in self.matching_words) + r')\b'
